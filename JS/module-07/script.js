...
const getTotalBalance = users => {
  let result = users.reduce((sumBalance, user) => {
...
const getUniqueSkills = users => {
  ...
  let skillsArr = users
    .map(filterBySkills => filterBySkills.skills)
    .reduce((newArr, newSkill) => {
      let filterArr = newSkill.filter(original => !newArr.includes(original));
