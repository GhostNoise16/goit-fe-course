...
const titleNoteForm = document.querySelector('input[name="note_title"]');
const bodyNoteForm = document.querySelector('textarea[name="note_body"]');
...
const submitNoteForm = event => {
  ...
  if (titleNoteForm.value === '' || bodyNoteForm.value === '') {
    return alert('Необходимо заполнить все поля!');
  }

  const submitedNote = {
    id: generateUniqueId(),
    title: titleNoteForm.value,
    body: bodyNoteForm.value,
    priority: PRIORITY_TYPES.LOW,
  };
  ...
  addListItem(noteList, submitedNote);
};
...
noteForm.addEventListener('submit', submitNoteForm);
noteList.addEventListener('click', deleteNotes);
